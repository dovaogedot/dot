...
extension (path: String) {
  ...
  def normalize: String = {
    val drive = if path.length >= 2 && path(0).isLetter && path(1) == ':' then path.take(2) else ""
  ...
  def dirname: String = {
    ...
    if i < 0 then "." else if i == 0 then "/" else path.take(i)
  }
  ...
  def resolvePath(home: String): String = {
    ...
    if s == "~" then
      ...
      home / s.drop(2)
    else if s.isAbsolutePath then
      s.normalize
    else
      System.getProperty("user.dir").toSlash / s
  }
  ...
  def contractTarget(home: String): String =
    if path == home then
      "~"
    else if path.startsWith(home + "/") then
      "~/" + path.drop(home.length + 1)
    else
      path
  ...
  def repoPathFor: String =
    if path.startsWith("~/") then
      ...
      "_root/" + path.stripPrefix("/").replaceFirst(":", "")
}
