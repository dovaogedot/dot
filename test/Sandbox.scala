...
def readText(path: String): IO[String] =
  readTextIfExists(path).flatMap: content =>
 ...
 */
final case class Sandbox(root: String, bin: String) {
  val home      = root / "home"
  ...
  def dot(args: String*): IO[String] = tryDot(args*) >>= succeeded(s"dot ${args.mkString(" ")}")

  /** Runs the binary and reports how it went, whatever the exit status. */
  def tryDot(args: String*): IO[Run] = exec(bin, args.toList)
  ...
  def diverge(rel: String, onHost: String, onRemote: String): IO[Unit] =
    writeText(home / rel, onHost)
      *> writeText(filesDir / rel, onRemote)
  ...
  def breakPushes: IO[Unit] = git("config", "remote.origin.pushurl", root / "nowhere").void
  ...
  private def prepared: IO[Sandbox] =
    ensureDir(home)
      *> writeText(root / "gitconfig", Sandbox.gitIdentity)
      *> gitIn(root, "init", "--quiet", "--bare", remote)
      *> probe.as(this)
  ...
  private def probe: IO[Unit] =
    exec("sh", List("-c", "printf %s \"$HOME\"")).flatMap: run =>
      IO.raiseUnless(run.out == home)(HarnessError.Leaky(run.out))

  private def gitIn(dir: String, args: String*): IO[String] = {
    val out = exec("git", "-C" :: dir :: args.toList) >>= succeeded(s"git ${args.mkString(" ")}")
    out.map(_.trim)
  }
...
object Sandbox {
  ...
  private def binary: IO[String] =
    IO.fromEither(homeDir).flatMap { home =>
      val bin = envGet("DOT_BIN").getOrElse(home / ".local/bin/dot")
      exists(bin).ifM(IO.pure(bin), IO.raiseError(HarnessError.NoBinary(bin)))
    }
  ...
  def make: Resource[IO, Sandbox] = {
    for
      ...
      sb   <- Resource.eval(Sandbox(root.toString, bin).prepared)
    yield sb
  }
}
