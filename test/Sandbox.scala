...
enum HarnessError extends RuntimeException with NoStackTrace {
  ...
  /** There is no dot binary at path. The suites need an installed binary, or DOT_BIN pointing at one. */
...
/**
 * An isolated home, dot data directory and bare remote for one test. Every process started through it
...
final case class Sandbox(root: Path, bin: Path) {
  ...
  /** Runs dot sync on a pseudo-terminal and types keys at its prompts. Returns everything it printed. */
